...
phase_network(){
  ...
  if [[ "$MODE" == "MASTER" ]]; then
    read -rp "${ROLE_LABEL} Enter virtual IP (VIP) for HA: " HA_VIP
    read -rp "${ROLE_LABEL} Enter password for Keepalived auth: " HA_PASS
  fi
}
...
EOF
  ...
  systemctl enable --now keepalived &>/dev/null && echo -e " ${OK}started${RESET}" || { echo -e " ${ERROR}fail${RESET}"; exit 1; }

  if [[ "$MODE" == "MASTER" ]]; then
    echo -n "${ROLE_LABEL} ${INFO}Waiting up to 30s for VIP to bind…${RESET}"
    for i in $(seq 1 30); do
      if ip addr show "$TRAFFIC_IF" | grep -qw "$HA_VIP"; then
        echo -e " ${OK}bound${RESET}"
        break
      fi
      sleep 1; printf "."
    done
    if ! ip addr show "$TRAFFIC_IF" | grep -qw "$HA_VIP"; then
      echo -e "\n${ROLE_LABEL} ${ERROR}timeout, VIP did not bind${RESET}"
      exit 1
    fi
  fi
}
